...
int ha_helios::set_fields_from_helios(uchar *buf,
  ...
  field_pack_.make_mysql_table_row(read_buf, read_buf_size);
  ...
  const auto nullFlags = field_pack_.get_null_flags();
  ...
  for (Field **field = table->field; *field; field++) {
    if (columnIndex >= field_pack_.get_row_size()) break;
    const auto mysqlFieldValue = field_pack_.get_column_of_row(columnIndex++);
