#!/bin/bash
# M1 final check at default kC_materialise=8: rebuild, restart mysqld (no
# C_MATERIALISE env -> code default 8), validate full suite on live SF1 server.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; cd "$ROOT"
MYSQL="$ROOT/build/runtime_output_directory/mysql"; SOCK=/tmp/mysql.sock; DB=benchbase
OUT=/tmp/m1_final3; rm -rf "$OUT"; mkdir -p "$OUT"
[ -f /tmp/mysql.pid ] && kill -9 "$(cat /tmp/mysql.pid)" 2>/dev/null || true; sleep 2
bash scripts/build_partial.sh 2>&1 | tail -1
...
echo "C_MATERIALISE env: $(tr '\0' '\n' < /proc/"$(cat /tmp/mysql.pid)"/environ | grep -i C_MATERIALISE || echo 'ABSENT -> code default 8')"
