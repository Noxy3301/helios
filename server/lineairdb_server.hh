...
class LineairDBServer : public TcpServer {
 ...
 private:
  ...
  // thread. classify_rpc() only ever admits the two stateless opcodes as
  // kFast, so every conversational or heavy opcode migrates before reaching
  // a Dispatcher -- nothing here ever opens a stored transaction, so the
  // disconnect cleanup handle_client() runs (abort_all_and_end +
  // ReleaseMasstreeThreadEpoch) has no counterpart on this path.
