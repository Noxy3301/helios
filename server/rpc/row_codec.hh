...
// schema-driven by the column's declared FieldType (a typed int whose LE bytes
// happen to be ASCII digits, 808464432 == "0000", would otherwise be silently
// mis-parsed), and the raw bytes are never sniffed.
// ---------------------------------------------------------------------------

/**
 * @brief Append the exact val_str ASCII of a cell to `*out`.
 *
 * @details Verbatim for kUntyped (and empty cells); reformatted from the
 * fixed-width binary for a typed cell. Matches the engine's gather formatting
 * byte-for-byte.
 */
void format_typed_cell(helios::storage::pax::FieldType type, int scale,
                       std::string_view value, std::string* out);

/**
 * @brief Canonical val_str ASCII view of a cell.
 *
 * @details Returns the cell itself for kUntyped or an empty cell; otherwise
 * formats into `buf` (whose storage backs the returned view).
 */
std::string_view typed_key_view(std::string_view value,
                                helios::storage::pax::FieldType type, int scale,
                                std::string& buf);
