...
// Evaluates a pushed FilterExpr against a single stored row.
...
class PredicateEvaluator {
 public:
  // Parse a stored row: [null_flags][col_0][col_1]...[col_N-1]
   ...
   */
  bool set_row_from_pax(const helios::storage::pax::PaxGroup& group, uint32_t slot,
 ...
 private:
  ...
  // (INT -> int, DATE -> YYYYMMDD int); an untyped column keeps the
  // strtoll/strtod ASCII path. Field index for column c is c+1.
  const helios::storage::pax::TableSchema* schema_ = nullptr;
