...
void LineairDBRpc::buildExecuteReadPlanResponse(
    ...
    for (const auto& step : request.steps()) {
        ...
        if (step.index_name().empty()) {
            ...
            // Phase-8 Phase B: aggregate server-side and emit group rows instead
            // of the raw rows. The proxy disables projection on aggregate steps,
            // so extract_value_column reads original field indices. range_versions
            // still cover the scanned footprint (OCC unaffected). Falls back to
            // per-row emit if the spec is not server-aggregatable.
