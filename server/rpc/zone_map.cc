...
// database-lifetime objects (a table reload requires a server restart), so a
// pointer is never freed and re-allocated under a live process; even if it
// were, a mismatched locked kind would make cells fail ClassifyCell ->
// ZS_INVALID -> no prune (never a wrong result), and the write_counter check
...
// optimized), and there is no min/max to prune with.
StripZone build_strip(PaxGroup* grp, uint32_t column, ZKind kind, int scale,
    ...
    for (uint32_t slot = 0; slot < PaxGroup::kRows; ++slot) {
        ...
        // columns, which never reach a prunable zone: the compare_type gate
        // rejects them.)
...
bool ClassifyCell(std::string_view s, ZKind* k, int* scale, int64_t* val) {
    ...
    if (seen_dot) {
        ...
        // exactly representable, which covers the fractional decimals in use.
...
bool GetColumnZones(PaxStore* store, uint32_t column, size_t n_groups,
    ...
    // Typed cells hold fixed-width LE binary that ClassifyCell would mis-read as
    // ASCII (e.g. a typed int whose bytes are digit characters), yielding a
    // bogus min/max and an incorrect prune. Zone maps are perf-neutral on
    // TPC-H, so typed columns are simply never pruned (skipping a prune is
    // always correct). Field index for MySQL column c is c+1.
    const auto& field_kind = store->schema().field_kind;
    if (static_cast<size_t>(column) + 1 < field_kind.size() &&
        field_kind[column + 1] != LineairDB::Pax::FK_UNTYPED)
        return false;
    ...
    // the success case (the quiescence check proves they match) and keeps this
    // module independent of the executor's snapshot layout.
    ...
    for (size_t g = 0; g < n_groups; ++g) {
        ...
        if (z.state != ZS_ABSENT && z.wc == wc1) continue;  // cached and fresh
        ...
        // strict complement to the executor's quiescence check, not a
        // replacement for it.
        const uint64_t wc2 =
            grp->write_counter.load(std::memory_order_acquire);
        if (wc2 != wc1) {
