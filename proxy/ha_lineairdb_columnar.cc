...
#define LDB_COL_REJECT(reason) \
  ...
  std::vector<int> tab_nest;       // per tab: sj_infos index or -1
  for (Table_ref *tl = qb->leaf_tables; tl != nullptr; tl = tl->next_leaf) {
    fprintf(stderr, "[LDBC] leaf '%s' outer=%d", tl->alias, (int)tl->outer_join);
    for (Table_ref *emb = tl->embedding; emb != nullptr; emb = emb->embedding)
      fprintf(stderr, " emb(sj=%d aj=%d cond=%d)", (int)emb->is_sj_nest(),
              (int)emb->is_aj_nest(), emb->join_cond() != nullptr);
    fprintf(stderr, "\n");
    if (tl->table == nullptr || tl->table->s == nullptr)
