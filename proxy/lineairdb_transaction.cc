...
void LineairDBTransaction::execute_read_plan(
  ...
  for (size_t i = 0; i < result.steps.size() && i < steps.size(); ++i) {
    ...
    if (step.index_name.empty()) {
      ...
      bool grouped_semijoin_agg_step = false;
      if (agg_step) {
        for (const auto& gs : grouped_semijoins_) {
          if (gs.inner_table_key == step.table_name) {
            grouped_semijoin_agg_step = true;
            break;
          }
        }
      }
